    ...
    return (
        <div className="w-full min-h-screen bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-50 selection:bg-slate-200 dark:selection:bg-slate-800 transition-colors duration-200">
            ...
            <section className="max-w-7xl mx-auto px-6 sm:px-12 lg:px-16 mb-6">
                <div className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm dark:bg-slate-900 dark:border-slate-800 flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
                    ...
                    <div className="relative flex-grow">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-slate-50/50 border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-slate-300 dark:bg-slate-950 dark:border-slate-800 dark:text-white dark:placeholder:text-slate-600 dark:focus:border-slate-700 transition-colors"
                        />
                    </div>
                    ...
                    <div className="relative min-w-[160px]">
                        <select
                            ...
                            className="w-full pl-4 pr-10 py-2 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-700 appearance-none focus:outline-none focus:border-slate-300 dark:bg-slate-950 dark:border-slate-800 dark:text-slate-300 dark:focus:border-slate-700 transition-colors cursor-pointer font-medium"
                        >
                            {categories.map((cat) => (
                                <option key={cat} value={cat} className="dark:bg-slate-950 dark:text-white">{cat}</option>
                            ))}
                        </select>
                    ...
                    <div className="relative min-w-[140px]">
                        ...
                        >
                            {levels.map((lvl) => (
                                <option key={lvl} value={lvl} className="dark:bg-slate-950 dark:text-white">{lvl}</option>
                            ))}
                        </select>
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3.5 text-slate-400 dark:text-slate-500">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {filteredAndSortedCourses.map((course) => (
                            <div
                                key={course.id}
                                className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden flex flex-col justify-between dark:bg-slate-900 dark:border-slate-800 transition-colors"
                            >
                                {/* Top Banner Image Fallback with dynamic Initials */}
